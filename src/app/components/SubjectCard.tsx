...
const SubjectCard: React.FC<SubjectCardProps> = ({
  ...
  return (
    <>
      ...
      >
        <div
          className={`div h-32 w-36 sm:w-52 ${
            isDarkMode ? "bg-gray-800" : "bg-slate-50"
          } m-auto rounded-[1em] relative group p-2 z-0 overflow-hidden flex items-center justify-center shadow-md`}
        >
          {colorToRender === "green" ? (
            <>
          ...
          <button className="w-full text-[0.8em] absolute bottom-2">
            <span
              className={`text-center relative before:h-[0.16em] before:absolute before:w-full before:content-[''] hover:underline ${
                isDarkMode ? "text-white" : "text-[#0d151c]"
              }`}
            >
              Aprender
            </span>
          ...
          <h1
            className={`z-20 font-bold font-Poppin text-base sm:text-[1.4em] text-center ${
              isDarkMode ? "text-white" : "text-[#0d151c]"
            }`}
          >
            {title}
