...
const SubjectCard: React.FC<SubjectCardProps> = ({ title, colorToRender }) => {
      ...
      {colorToRender === "green" ? (
        ...
        >
          <span className="text-center relative before:h-[0.16em] before:absolute before:w-full before:content-[''] hover:underline">
            <Link href={`/${lastText === 'cognitive' ? 'cognitive' : lastText === 'verbal' ? 'verbal' : lastText === 'mathematics' ? 'mathematics' : ''}/${normalizeTitle(title)}`}>
              Aprender
            </Link>
          </span>
